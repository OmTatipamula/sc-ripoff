...
// ParseFailureOptions:
// - The first token is assumed to be the service name (if it does not contain '=' then it is the service name)
// - Then, key/value pairs are expected (keys ending with '=' and followed by a value).
void ParseFailureOptions(const std::vector<std::string>& args, FailureOptions &opts) {
    ...
    size_t index = 0;
    // If the first token does not contain '=' then it is the service name.
...
// failure: Configures the failure actions for a service using ChangeServiceConfig2A.
// This function builds a SERVICE_FAILURE_ACTIONS structure based on the options parsed.
void failure(const FailureOptions &opts) {
    ...
    SC_HANDLE hService = OpenServiceA(hSCManager, opts.serviceName.c_str(), SERVICE_CHANGE_CONFIG);
    if (!hService) {
    ...
    // Build the SERVICE_FAILURE_ACTIONS structure.
    SERVICE_FAILURE_ACTIONS sfa;
    sfa.dwResetPeriod = opts.reset; // seconds
    // Use const_cast to convert const char* to LPSTR.
    ...
    if (opts.actions == "\"\"" || opts.actions.empty()) {
        ...
        // The actions string is expected to be in the form:
        //   e.g., "restart/5000/reboot/10000"
        ...
        for (size_t i = 0; i < tokens.size(); i += 2) {
            ...
            // Map the action type string to SC_ACTION type.
            if (actionType == "run") {
